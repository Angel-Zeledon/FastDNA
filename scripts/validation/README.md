...
|---|---|---|
| `kmc3_equivalence.py` | k-mer counting: both engines (`--k`, `--engine`) and both conventions (`--no-canonical`) | KMC3 3.2.4, **exact equality** |
...
| `similarity_vs_kmc_tools.py` | exact Jaccard and containment between tables, both widths (`--k`) | KMC3's own set operations, **exact equality** |
